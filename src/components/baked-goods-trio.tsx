...
export function BakedGoodsTrio() {
  return (
    <section className="max-md:hidden border-t border-[var(--color-border)] bg-[var(--color-cream)] px-5 py-14 font-dm-sans sm:px-8 sm:py-20 lg:px-12 lg:py-24">
      <div className="mx-auto w-full max-w-[min(100%,24rem)] py-1 sm:max-w-3xl sm:py-0 md:max-w-4xl lg:max-w-5xl">
        <div className="grid grid-cols-3 items-center gap-2 sm:gap-5 md:gap-7 lg:gap-9">
          {PHOTOS.map((item) => (
            <div
              key={item.src}
              className={`min-w-0 w-full origin-center transition-transform duration-300 ease-out hover:scale-[1.03] ${item.tilt}`}
            >
